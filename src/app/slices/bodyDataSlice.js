...
const bodyDataSlice = createSlice({
    ...
    initialState,
    reducers: {
        addBodyData: (state, action) => {
            state.bodyDatas.push(action.payload);
        },
        removeBodyData: (state, action) => {
            state.bodyDatas = state.bodyDatas.filter(bodyData => bodyData.uuid !== action.payload);
        }
    }
});

export const selectBodyDatas = (state) => state.bodyDatas;

export const { addBodyData, removeBodyData } = bodyDataSlice.actions;
